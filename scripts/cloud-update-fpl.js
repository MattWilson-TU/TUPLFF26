...
async function downloadAndUpdateFPLData() {
  try {
    ...
    const lastWeekResult = await prisma.gameweekPlayerPoints.aggregate({
      _max: { gameweekId: true }
    })
    const lastWeekWithData = lastWeekResult?._max?.gameweekId || null

    let gameweeksToDownload = []
    if (!lastWeekWithData) {
      gameweeksToDownload = availableGameweeks
    } else {
      gameweeksToDownload = availableGameweeks.filter(gw => gw >= lastWeekWithData)
      if (!availableGameweeks.includes(lastWeekWithData)) {
        gameweeksToDownload = [lastWeekWithData, ...gameweeksToDownload]
          .sort((a, b) => a - b)
      }
    }

    console.log(
      `🧭 Last week with data: ${lastWeekWithData ?? 'none'}, ` +
      `downloading: ${gameweeksToDownload.join(', ') || 'none'}`
    )
    
    // Download live data for each required gameweek
    for (const gw of gameweeksToDownload) {
