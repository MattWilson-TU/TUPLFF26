...
const bidSchema = z.object({
  lotId: z.string(),
  bidType: z.enum(['SP', '+0.5', '+1.0', '+2.0', 'OPEN+0.5', 'ALL_IN']),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { lotId, bidType } = bidSchema.parse(body)
    ...
    const currentHighest = lot.bids[0]?.amountHalfM || 0
    ...
    switch (bidType) {
      case 'SP':
        ...
        break
      case '+0.5':
        bidAmount = currentHighest > 0 ? currentHighest + 1 : startingPrice + 1
        break
      case '+1.0':
        bidAmount = currentHighest > 0 ? currentHighest + 2 : startingPrice + 2
        break
      case '+2.0':
        bidAmount = currentHighest > 0 ? currentHighest + 4 : startingPrice + 4
        break
      case 'OPEN+0.5':
        // Explicitly open above SP if no current bid, otherwise behaves like +0.5
        bidAmount = currentHighest > 0 ? currentHighest + 1 : startingPrice + 1
        break
      case 'ALL_IN': {
        // Calculate manager's remaining budget (Budget - Spent)
        // Get the current auction to find spent amounts
        ...
        const remainingHalfM = startingBudgetHalfM - totalSpentHalfM
        
        // Must be at least starting price
        bidAmount = Math.max(startingPrice, remainingHalfM)
        break
      }
      default:
        return NextResponse.json(
    ...
    if (bidAmount > remainingBudgetHalfM) {
      return NextResponse.json(
